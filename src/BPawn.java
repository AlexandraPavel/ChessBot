...
public class BPawn extends Piece {
    ...
    public boolean check_enpassant(Square square) {
        if(square.piece instanceof WPawn) {
            WPawn pawn = (WPawn) square.piece;
            if(pawn.en_passant) {
                return true;
            }
        }
    ...
    public IndexPair move(Square[][] board, int x, int y) {
        ...
        if (indices.x != -1) {
            ...
            if (board[indices.x][indices.y].piece instanceof BRook)
                System.out.println("move " + (char) (y + 97) + (8 - x) +
                        (char) (indices.y + 97) + (8 - indices.x) + "r");
            else if (board[indices.x][indices.y].piece instanceof BQueen)
                System.out.println("move " + (char) (y + 97) + (8 - x) +
                        (char) (indices.y + 97) + (8 - indices.x) + "q");
            else if (board[indices.x][indices.y].piece instanceof BBishop)
                System.out.println("move " + (char) (y + 97) + (8 - x) +
                    (char) (indices.y + 97) + (8 - indices.x) + "b");
            else if (board[indices.x][indices.y].piece instanceof BKnight)
                System.out.println("move " + (char) (y + 97) + (8 - x) +
                    (char) (indices.y + 97) + (8 - indices.x) + "n");
            else 
            System.out.println("move " + (char) (y + 97) + (8 - x) +

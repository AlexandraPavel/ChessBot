...
public class WKing extends Piece {
    ...
    public IndexPair take_or_enter_attack(Square[][] board, int x, int y, ArrayList<IndexPair> moves,IndexPair check_source) {
        ...
        for (i = 0; i < 8; i++)
            for (j = 0; j < 8; j++)
                if (board[i][j].piece.colour.compareTo("white") == 0) {
    ...
    public ArrayList<IndexPair> verify_moves(Square[][] board, int x, int y, ArrayList<IndexPair> illegal_moves, ArrayList<IndexPair> source_array) {
        ...
        for (IndexPair move : moves) {
            board[move.x][move.y] = new Square(move.x, move.y, new WKing(((WKing)board[x][y].piece).checked));
            board[x][y] = new Square(x, y);
            ArrayList<ArrayList<IndexPair>> is_check_result = ((WKing) board[move.x][move.y].piece).is_in_check(board, move.x, move.y);
            board[x][y] = new Square(x, y, new WKing(((WKing)board[move.x][move.y].piece).checked));
